...
import nl.mtvehicles.core.infrastructure.libs.worldguardwrapper.shaded.javassist.CtNewConstructor;
...
public class StubGenerator implements Translator {
   ...
   private static final String sampleClass = "nl.mtvehicles.core.infrastructure.libs.worldguardwrapper.shaded.javassist.tools.rmi.Sample";
   ...
   public void start(ClassPool pool) throws NotFoundException {
      this.classPool = pool;
      CtClass c = pool.get("nl.mtvehicles.core.infrastructure.libs.worldguardwrapper.shaded.javassist.tools.rmi.Sample");
      this.forwardMethod = c.getDeclaredMethod("forward");
      this.forwardStaticMethod = c.getDeclaredMethod("forwardStatic");
      this.proxyConstructorParamTypes = pool.get(new String[]{"nl.mtvehicles.core.infrastructure.libs.worldguardwrapper.shaded.javassist.tools.rmi.ObjectImporter", "int"});
      this.interfacesForProxy = pool.get(new String[]{"java.io.Serializable", "nl.mtvehicles.core.infrastructure.libs.worldguardwrapper.shaded.javassist.tools.rmi.Proxy"});
      this.exceptionForProxy = new CtClass[]{pool.get("nl.mtvehicles.core.infrastructure.libs.worldguardwrapper.shaded.javassist.tools.rmi.RemoteException")};
   }
   ...
   private CtClass produceProxyClass(CtClass orgclass, Class<?> orgRtClass) throws CannotCompileException, NotFoundException {
      ...
      if (!Modifier.isAbstract(modify) && !Modifier.isNative(modify) && Modifier.isPublic(modify)) {
         ...
         CtField f = new CtField(this.classPool.get("nl.mtvehicles.core.infrastructure.libs.worldguardwrapper.shaded.javassist.tools.rmi.ObjectImporter"), "importer", proxy);
         f.setModifiers(2);
