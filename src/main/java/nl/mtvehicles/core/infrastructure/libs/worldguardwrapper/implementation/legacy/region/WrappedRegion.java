package nl.mtvehicles.core.infrastructure.libs.worldguardwrapper.implementation.legacy.region;
...
public class WrappedRegion implements IWrappedRegion {
      ...
      if (this.handle instanceof ProtectedCuboidRegion) {
         ...
         return new IPolygonalSelection() {
            public Set<Location> getPoints() {
               return (Set)WrappedRegion.this.handle.getPoints().stream().map((vector) -> new BlockVector(vector.toVector())).map((vector) -> WorldGuardVectorUtilities.fromBlockVector(WrappedRegion.this.world, vector)).collect(Collectors.toSet());
            }
   ...
   public boolean contains(Location location) {
      return this.handle.contains(WorldGuardVectorUtilities.toBlockVector(location));
   }
