...
@Examples({"if player is the vehicle owner of {_car}:"})
@Since({"2.5.5"})
public class CondIsOwner extends Condition {
   ...
   static {
      Skript.registerCondition(CondIsOwner.class, new String[]{"[player] %player% is [the] [mtv] vehicle owner of %mtvehicle%", "[player] %player% (isn't|is not) [the] [mtv] vehicle owner of %mtvehicle%"});
   }
}
