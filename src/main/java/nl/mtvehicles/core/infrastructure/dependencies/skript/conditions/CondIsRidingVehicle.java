...
@Examples({"if player {_p} is driving an mtv vehicle:", "if player is not driving mtv vehicle:"})
@Since({"2.5.6"})
public class CondIsRidingVehicle extends Condition {
   ...
   public boolean check(Event event) {
      boolean check = this.player.getSingle(event) != null && isRidingVehicle((Player)this.player.getSingle(event));
      if (!this.isNegated()) {
         return check;
      } else {
         return !check;
   ...
   static {
      Skript.registerCondition(CondIsRidingVehicle.class, new String[]{"[player] %player% is (driving|riding) [(a|an)] mtv vehicle", "[player] %player% (isn't|is not) (driving|riding) [(a|an)] mtv vehicle"});
   }
}
