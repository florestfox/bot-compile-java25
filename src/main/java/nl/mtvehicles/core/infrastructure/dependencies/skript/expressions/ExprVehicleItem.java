...
public class ExprVehicleItem extends SimpleExpression<ItemStack> {
   ...
   protected ItemStack[] get(Event event) {
      ...
      } else if (!VehicleUtils.vehicleUUIDExists((String)this.identifier.getSingle(event))) {
   ...
   static {
      Skript.registerExpression(ExprVehicleItem.class, ItemStack.class, ExpressionType.SIMPLE, new String[]{"[a[n]] mtv vehicle item (by|with) license [plate] %string%", "[a[n]] aesthetic mtv vehicle item (by|with) (uuid|UUID) %string%"});
   }
}
