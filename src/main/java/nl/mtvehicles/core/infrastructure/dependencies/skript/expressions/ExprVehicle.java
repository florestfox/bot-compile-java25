...
@Examples({"set {_car} to mtv vehicle with license plate \"DF-4J-2R\"", "set {_helicopter} to player's driven mtv vehicle", "set {_car} to a new mtv vehicle with UUID \"C4UQZJ\" and owner player"})
public class ExprVehicle extends SimpleExpression<Vehicle> {
   ...
   protected Vehicle[] get(Event event) {
      if (this.pattern == 0) {
         return this.licensePlate == null ? null : new Vehicle[]{VehicleUtils.getVehicle((String)this.licensePlate.getSingle(event))};
      } else if (this.pattern == 1) {
         return new Vehicle[]{VehicleUtils.getDrivenVehicle((Player)this.player.getSingle(event))};
      } else if (!VehicleUtils.vehicleUUIDExists((String)this.uuid.getSingle(event))) {
   ...
   static {
      Skript.registerExpression(ExprVehicle.class, Vehicle.class, ExpressionType.SIMPLE, new String[]{"[a[n]] mtv vehicle [with license [plate] %-string%]", "%player%'s driven mtv vehicle", "[a] new[ly] [created] mtv vehicle (by|with) (uuid|UUID) %string% [and] [(by|with)] owner %offlineplayer%", "[a] new[ly] [created] mtv vehicle (by|with) (uuid|UUID) owner %offlineplayer% [and] [(by|with)] (uuid|UUID) %string%"});
   }
}
